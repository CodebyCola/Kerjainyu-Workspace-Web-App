...
export class ProjectMemberService {
    async addMember(project_id: number, input: CreateProjectMemberSchema,) {
        return await projectMembersRepository.create({ project_id: project_id, role: input.role, user_id: input.userId })
    }
    async removeMember(project_id: number, user_id: number, leader_id: number) {
        ...
        const leader = await projectMembersRepository.findByProjectAndUser(project_id, leader_id)
    ...
    async transferLeader(project_id: number, current_leader_id: number, new_leader_id: number) {
        const currentLeader = await projectMembersRepository.findByProjectAndUser(project_id, current_leader_id)
        if (!currentLeader || currentLeader.role !== "leader") {
            throw new ForbiddenError("Only the current leader can transfer leadership")
        }
        const newLeader = await projectMembersRepository.findByProjectAndUser(project_id, new_leader_id)
        if (!newLeader) {
        ...
        return await db.transaction().execute(async (trx) => {
            await projectMembersRepository.demoteToMember(project_id, currentLeader.id, trx);
            return await projectMembersRepository.promoteToLeader(project_id, newLeader.id, trx)
        })
    }
}